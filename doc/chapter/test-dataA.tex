...
\begin{lstlisting}[caption={Snippet usato per il calcolo del GAP per il detaset A}, label={code:gap-dataset-a}, language=bash]
    ...
    gp_file=out_potenziale_gap.xml
...
Il valore di \texttt{e0} è stato impostato su zero in quanto non abbiamo a disposizione i valori di energia per atomi isolati, per questi dati si necessita di una simulazione di QE con gli stessi potenziali usati per creare il dataset A, potenziali che non abbiamo a disposizione. 
Questo è certo generare problemi di accuratezza del risultato.
