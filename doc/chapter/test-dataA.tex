...
Abbiamo provato ad effettuare una simulazione di tipo NVE, costruita al seguente modo. Dai file di QE, sia per il set di training che per quello di test, da questo deriva la nomenclatura successiva, abbiamo estratto la configurazione spaziale iniziale degli atomi.
...
Seguendo la struttura descritta precedentemente abbiamo impostato la simulazione con i seguenti parametri, timestep pari a $4.8$fs, come quello usato in Quantum Espresso, e lo stesso numero di step.
...
Abbiamo provato a reperire i valori delle velocità iniziali dal file di output di QE, ma non essendo presenti, le abbiamo estratte da una distribuzione di Maxwell-Boltzman per una temperatura di $3000$K.
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{./img/andamento_nve-traindata.png}
    \caption{Grafico dell'andamento temporale per i valori di energia e temperatura}
    \label{fig:andamento_nve-traindata}
\end{figure}

possiamo vedere \ref{fig:andamento_nve-traindata} che, anche se per valori leggermente più contenuti rispetto ai dati di test, sia l'energia che la temperatura divergono a valori spaventosamente elevati. 
L'energia potenziale anche se per un range relativamente ampio oscilla in modo limitato.

\subsubsection{Test set}

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{./img/andamento_nve-testdata.png}
    \caption{Grafico dell'andamento temporale per i valori di energia e temperatura}
...
In questa sezioni ci poniamo di modificare a turno vari parametro o combinazioni di tali in modo da poter identificare quale sia il problema.


\subsection{Temperatura}

Per prima cosa abbiamo provato a modificare la temperatura dalla quale venivano estratti i valori di velocità. 
Si è impostato questo valore a $0$K. 
Il timestep non è stato modificato.

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{./img/andamento_nve-traindata_0k.png}
    \caption{Grafico dell'andamento temporale per i valori di energia e temperatura}
    \label{fig:andamento_nve-traindata_0k}
\end{figure}

Si nota \ref{fig:andamento_nve-traindata_0k} che il risultato non è dissimile da quello ottenuto in precedenza, possiamo quindi escludere che sia un problema legato al valore di temperatura.

\subsection{Timestep}

Per questa analisi sono stati provati diversi valori di timestep, riportiamo solo lo step finale.
Come timestep si è impostato un valori di $10^{-5}$fs, e un totale di $50'000$ iterazioni, in modo da valutate la stabilità sul lungo periodo.


\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{./img/andamento_nve.png}
    \caption{Grafico dell'andamento temporale per i valori di energia e temperatura}
    \label{fig:andamento_nve}
\end{figure}

considerando questo risultato \ref{fig:andamento_nve} la prima cosa che si vede è una diminuzione del range in cui i valori di temperatura e di energia variano.
Si potrebbe pensare che il problema sia risolto, ma possiamo notare che entrambe mantengono la medesima forma vista in precedenza, possiamo quindi presumere che stiano comunque divergendo, ma ad un rate estremamente basso.
Questo ci permette di escludere anche il timestep come fonte di problemi.

\subsection{Forze}

Passiamo adesso ad esaminare il comportamento delle forze.
Anche in questo caso sono state svolte diverse prove, riportiamo solo la seguente, timestep di $10^{-1}$fs (valori più piccoli porterebbero solo a ritardare la divergenza dei valori), $5'000$ step e temperatura di $3000$K.
In questo caso sono state estratte anche i valori delle forze oltre che quelli di temperatura ed energia.


\begin{figure}[h]
    \centering
    \includegraphics[scale=0.4]{./img/andamento_energia_nve.png}
    \caption{Grafico dell'andamento temporale per i valori di energia e temperatura}
    \label{fig:andamento_energia_nve}
\end{figure}


\begin{figure}[h]
    \centering
    \includegraphics[scale=0.4]{./img/andamento_forze_nve.png}
    \caption{Grafico dell'andamento temporale per i valori delle forze}
    \label{fig:andamento_forze_nve}
\end{figure}

dove possiamo notare che in corrispondeza dell'esplosione delle forze \ref{fig:andamento_forze_nve} sia ha anche quello di energia e temperatura \ref{fig:andamento_energia_nve}.

Per ricercare una spiegazione di questo comportamento, abbiamo estratto le configurazioni spaziali degli atomi ogni $200$ step.
Per mostrare cosa sta succedendo riportiamo e confrontiamo la configurazione allo step $200$ e allo step $4800$.
   ...
   \begin{figure}[!h]
        ...
        \begin{subfigure}[c]{0.46\textwidth}
            \centering
            \includegraphics[scale=0.6]{./img/000200_configurazione_spaziale.png}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.46\textwidth}
            \centering
            \includegraphics[scale=0.6]{./img/004800_configurazione_spaziale.png}
        \end{subfigure}
        \caption{Configurazione spaziale dopo 200 e 4800 step}
        \label{fig:configurazione-spaziale-200-4800-step}
    \end{figure}

Possiamo vedere che nella parte finale (anche per gli step che non sono stati riportati) gli atomi tendano ad avvicinarsi e ad agglomerarsi, questo allontana la configurazione da quelle viste nella fase di training, questo comporta la predizione di valori di forza privi di significato.
Per provare a risolvere questo problema, nella prossima sezione, andremo ad addizionare un potenziale repulsivo.

\section{Potenziale di repulsione ZBL}
