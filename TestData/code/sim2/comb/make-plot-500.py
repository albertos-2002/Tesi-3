...
def genera_grafico_confronto():
    ...
    file_048 = "md_gap_test-500-comb.traj"
    file_qe = "../../test_data.extxyz"

    print(f"Caricamento traiettoria 4.8fs da: {file_48}...")
    traj_48 = Trajectory(file_48)  
    print(f"Caricamento traiettoria 0.48fs da: {file_048}...")
    ...
    index_48_dummy = np.arange(len_traj_48)
    index_48_ext = index_48_dummy *10
    index_048 = np.arange(len_traj_048)
    index_qe = np.arange(len(traj_qe))*10
    ...
    plt.scatter(index_48_ext, energie_48, color='blue', alpha=0.5, s=20, edgecolors='k', label='TimeStep 4.8fs, friction 1')
    plt.scatter(index_048, energie_048, color='red', alpha=0.5, s=20, edgecolors='k', label='TimeStep 0.48fs, friction 1')
    ...
    nome_grafico = "confronto_energie_comb-500.png"
